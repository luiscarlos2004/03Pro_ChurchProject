...
def viewpeople():
    ...
    conn.execute('''SELECT * FROM usersinformation''')

    resultPeople = [{"idu":person[0],"type_document":person[1],"document":person[2],"first_name":person[3],"middle_name":person[4],"first_last_name":person[5],"second_last_name":person[6],"address":person[7],"cellphone":person[8],"email":person[9],"company_id":person[10]} for person in conn]
    
    peopleArgInformation = [

        (

            personInformation["idu"],
            personInformation["type_document"],
            personInformation["document"],
            personInformation["first_name"],
            personInformation["middle_name"],
            personInformation["first_last_name"],
            personInformation["second_last_name"],
            personInformation["address"],
            personInformation["cellphone"],
            personInformation["email"],
            personInformation["company_id"]
...
@main.route('/peopleupdate/<id>', methods=['POST'])
def peopleUpdate(id):
    ...
    conn = mysql.connect.cursor()
    conn.execute('''UPDATE usersinformation set first_name = %s WHERE idu = %s''', (first_name,id,))
...
@main.route('/peopledelete/<string:id>', methods=['DELETE'])
def deletePeople(id):

    conn = mysql.connect.cursor()
    conn.execute('''DELETE FROM usersinformation WHERE idu = %s''', (id,))
    conn.connection.commit()
    conn.close()
